...
if __name__ == '__main__':
    ...
    for i in range(h, env.n-h+1):
        ...
        ddpg_actors.append(ddpg.Actor('actor' + str(i), session, n_action, speed,
                            state, state_next))
        ddpg_critics.append(ddpg.Critic('critic' + str(i), session, n_action,
                              ddpg_actors[i-h].eval_actions, ddpg_actors[i-h].target_actions,
                              state, state_next, reward))
        ddpg_actors[i-h].add_gradients(ddpg_critics[i-h].action_gradients)
        ddpg_actors_noise.append(OrnsteinUhlenbeckActionNoise(
            mu=ou_mus[i],
            sigma=ou_sigma[i],
            theta=ou_theta[i],
            dt=ou_dt[i],
            x0=ou_x0[i]))
        ddpg_memories.append(Memory(args.memory_size))
