...
def main():
    ...
    dqns = [DQN(env.action_space[agent_i].n, env.observation_space[
                agent_i].shape[0]) for agent_i in range(env.n)]
    ...
    for step in itertools.count():
        ...
        next_state, reward, done, info = env.step(agent_actions)
        print("reward", reward)

        for i in range(env.n):
            dqns[i].remember(state[i], actions[i], reward[
                             i], next_state[i], done)
            if step > 500:
                dqns[i].learn()
        ...
        if any(done):
            env.render()
            break
    ensure_directory_exists(os.path.splitext(
        options.weights_filename_prefix)[0])
    for i, dqn in enumerate(dqns):
        dqn_filename = options.weights_filename_prefix + str(i) + ".h5"
